...
import br.com.fatecjp.telas.novoRecorde;
...
public class AsteroidsCanvas extends GameCanvas implements Runnable, CommandListener
{
	...
	{		
		...
		if(iniciar != 4){
			...
			desenhaLimites(g);
			desenhaOvnis(g);			
			g.drawString("Pontos: "+this.pontuacao+" Tempo: "+this.tempo+" s", 35,260, Graphics.TOP | Graphics.LEFT);
			g.drawString("Ovnis: "+ getPosicao()+"  Ener: "+energia+"% Vel: " + velocidade*40 +" Km/h",35,270, Graphics.TOP | Graphics.LEFT);		
			...
			g.setColor(flag1);
			g.drawString("3", 113, 180, 0);	
			
			g.setColor(flag2);
			g.drawString("2", 123, 180, 0);
		
			g.setColor(flag3);
			g.drawString("1", 133, 180, 0);	
		}
		else{
			g.setColor(255,255,255);
			g.drawImage(back,0,0, Graphics.TOP | Graphics.LEFT);
	...
	{	
		...
		if(getPosicao() <= 0)
		{
			...
			setCurrent(novoRecorde.createInstance(pontuacao, tempo));
