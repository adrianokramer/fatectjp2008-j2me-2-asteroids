...
public class AsteroidsController extends GameCanvas implements Runnable, CommandListener
{
	...
	public static AsteroidsController getInstance() throws Exception {
		if (instance == null) {
			...
			throw new Exception("Instancia nï¿½o criada");
		}
		return instance;
	}
	...
	{
		//Inicializando variï¿½veis de jogo
		pausado = false;
		count = 0;
		started = 0;
		time = 0;
	...
	{	
		...
		if(playerShip.getEnergy() <= 0)
		{
			...
			//Local para testes... Este cï¿½digo deve ser removido desta posiï¿½ï¿½o
			setCurrent(SaveLocalHighScores.createInstance(score, time));
			//instance = null;
			...
			while(!pausado)
			{
				...
				{					
					...
					if(!pausado)
					{
						...
						for(int x=0; x<numOvnis; x++)
						{	
							//if(ship_y - ovniInimigo[x].getPy() > 10){ //Desenha os asteroids mais prï¿½ximos ï¿½ nave...
					...
					//System.out.println("Limite inferior: "+lower_limit_map+" Limite superior: "+upper_limit_map+" Posiï¿½ï¿½o da nava: "+spr_ship.getY());					
