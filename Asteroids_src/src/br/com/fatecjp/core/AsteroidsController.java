...
public class AsteroidsController extends GameCanvas implements Runnable, CommandListener
{
	...
	Sprite spr_ship, spr_red_explosion, spr_earth;
	...
	int rotation = 90; //Define o tipo de rotação inicial que será feita nos asteroids...
	//////////////////////////////////////////
	...
	public AsteroidsController(MIDlet md){
		...
		// Pega a largura máxima do visor.
		maxX = getWidth();
		
		// Ajusta a largura máxima do jogo. 
		// O jogo pode ter uma largura máxima de 240 px,
		// portanto, se o visor for maior que isso, ajusta para 240px.
		
		if (maxX > 240)
			maxX = 240;		

		// Pega a altura máxima do visor.
		maxY = getHeight();
	...
	public void startGame(boolean hard_reset)
	{
		...
		setState(); //Adiciona opções disponíveis
		...
		try {
			...
			img_ship 			= Image.createImage("/imagens/ship.png");
			spr_ship            = new Sprite(img_ship, 29, 33);
			
			// Define um sprite para a animação da explosão da nave:
			img_red_explosion	= Image.createImage("/imagens/red_explosions_25x25.png");
			spr_red_explosion   = new Sprite(img_red_explosion, 25, 25);
			
			// Define o planeta - Fim do jogo:
			img_earth     = Image.createImage("/imagens/earth.png");
			spr_earth     = new Sprite(img_earth, 100, 100);
			
			img_universe = Image.createImage("/imagens/universe.png");
		...
		// Associa as posições no objeto com o tiled_layer			
		int lin, col;			
		for(int i=0; i < 800;i++) {
			col  = i % 4;       
			lin  = i / 4;        
		...
		upper_limit_scr = lower_limit_scr - 250; //Comprimento do espaço jogável
	...
	private void carregaImagens()
	{
		try
		{
			img_asteroid1 = Image.createImage("/imagens/asteroid-1.png");
			img_asteroid2 = Image.createImage("/imagens/asteroid-2.png");
			img_asteroid3 = Image.createImage("/imagens/asteroid-3.png");
			img_asteroid4 = Image.createImage("/imagens/asteroid-4.png");
			img_asteroid5 = Image.createImage("/imagens/asteroid-5.png");
			img_asteroid6 = Image.createImage("/imagens/asteroid-6.png");
			img_asteroid7 = Image.createImage("/imagens/asteroid-7.png");
			img_asteroid8 = Image.createImage("/imagens/asteroid-8.png");
			//img_earth 	  = Image.createImage("/imagens/earth.png");
		}
		catch(IOException e)
		{
	...
	{	
		...
		if(started != 4){
			...
			text2.setLength(0);
			text2.append("Pontos: "+this.score);
			text3.setLength(0);
			text3.append("Ovnis: "+ getPosicao()+"  Ener: "+energia+"% Tempo: "+this.time+" s");//" Vel: " + velocidade*40 +" Km/h");		
		...
		//Carregando jogo...
		g.setColor(255, 0, 0);
		// Escreve a tecla pressionada e as posições x/y na tela.
	...
	{	
		...
		if(getPosicao() <= 0 && (ship_y<=spr_earth.getY() || spr_ship.collidesWith(spr_earth, true)))
		{
			pausado = true;
			started = 0;
			this.ini = false;
		...
		if(playerShip.getEnergy() <= 0)
		{
			...
			//Local para testes... Este código deve ser removido desta posição
			setCurrent(SaveLocalHighScores.createInstance(score, time));
			//instance = null;
			...
			while(!pausado)
			{
				...
				{					
					...
					if(!pausado)
					{
						...
						for(int x=0; x<numOvnis; x++)
						{	
							//if(ship_y - ovniInimigo[x].getPy() > 10){ //Desenha os asteroids mais próximos à nave...
					...
					//System.out.println("Limite inferior: "+lower_limit_map+" Limite superior: "+upper_limit_map+" Posição da nava: "+spr_ship.getY());					
					...
					// Escreve a tecla pressionada e as posições x/y na tela.
					g.drawString(text2.toString(), 5,25, Graphics.LEFT | Graphics.BOTTOM);	
					g.drawString(text3.toString(), 5,15, Graphics.LEFT | Graphics.BOTTOM);	
					
					flushGraphics(); //Atualizando gráficos
				...
				//System.out.println("MAP_LIMIT_LO:"+lower_limit_scr+" MAP_LIMIT_UP:"+upper_limit_scr+" Posição da nave:"+ship_y);
				
				 //Pausa para exibir a explosão
			    try{
					Thread.sleep(30);
				}
	...
	private void handleGame()
	{
		...
		*/
		...
		//Teste de colisões, após atualização das posições (ship x asteroids)
		//crashTest();
	...
	public boolean crashTest(Sprite ship, Sprite x) {
		...
		if(ship.collidesWith(x, true)) {
				...
				//spr_red_explosion.setFrame(10); //Apaga da tela o rastro da explosão
				return true;
		}
		else return false;
	}
